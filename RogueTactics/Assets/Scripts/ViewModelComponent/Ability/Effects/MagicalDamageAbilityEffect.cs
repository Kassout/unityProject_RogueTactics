...
public class MagicalDamageAbilityEffect : BaseAbilityEffect
{
    ...
    public override int Predict (WorldTile target)
    {
        ...
        int resistance = GetStat(attacker, defender, GetDefensiveStatNotification, 0);
        
        // Get the attackers base attack stat considering
        // mission items, support check, status check, and equipment, etc
        int magic = GetStat(attacker, defender, GetOffensiveStatNotification, 0);

        // Calculate base defense damage
        // TODO : add terrain modifiers + supports ?
        int effectiveDefense = resistance;
        
        // Calculate base damage
        int damage = magic + power - effectiveDefense;
